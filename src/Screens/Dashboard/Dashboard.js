...
class Dashboard extends Component {
  ...
  handleSubmitForm = data => {
    this.props.submitFormData(data, {
      SuccessCallback: res => {
        ...
        let ausCount = formDataArr.filter(data => data.country === "australia")
          .length;
        let canCount = formDataArr.filter(data => data.country === "canada")
          .length;
        let usaCount = formDataArr.filter(data => data.country === "usa")
          .length;
        let indCount = formDataArr.filter(data => data.country === "india")
          .length;
        let rusCount = formDataArr.filter(data => data.country === "russia")
          .length;
        let brzCount = formDataArr.filter(data => data.country === "brazil")
          .length;
        let data = [
        ...
        this.setState({
          ...
          data: data,
          pieData: pieData,
          lineChartData: this.state.data.map(d => {
            return {
              country: d.letter,
              value: formDataArr.filter(
                data => data.country.toLowerCase() == d.letter.toLowerCase()
              ).length
            };
          })
        });
  ...
  render() {
    console.log(this.state.formDataArr);
    return (
      <div className="container">
        ...
        <Chart
          formData={this.state.formDataArr}
          width={800}
          height={500}
